...
async function bootstrap() {
  const app = await NestFactory.create(ActivityLogModule);
  app.useGlobalInterceptors(
    new LoggingInterceptor(new AppLogger()),
    new TrackInterceptor(new AppLogger(), new Reflector()),
  );
  await app.listen(process.env.ACTIVITYLOG_PORT ?? 3003, () => {
    writeToConsole.log(
      `Request Demo Service is running on port ${process.env.ACTIVITYLOG_PORT ?? 3003}`,
    );
  });
}
bootstrap();
